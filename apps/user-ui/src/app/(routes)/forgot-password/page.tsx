...
const ForgotPassword = () => {
  ...
  const requestOtpMutation = useMutation({
    mutationFn: async ({ email }: { email: string }) => {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URI}/api//user-forgot-password`,
        { email }
      );
  ...
  const verifyOtpMutation = useMutation({
    mutationFn: async () => {
      if (!userEmail) return;
      const response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URI}/api/verify-user-forgot-password`,
  ...
  return (
    <div className='w-full py-10 min-h-[85vh] bg-white'>
      ...
      <p className='text-center text-lg font-medium py-3 text-[#0000099]'>
        ...
        <div className='md:w-[480px] p-8 bg-white shadow rounded-lg'>
          {step === "email" && (
            <>
              ...
              <p className='text-center text-gray-500 mb-4'>Go back to?{"  "}
                <Link href={"/login"} className='text-blue-500'>SignUp</Link>
              </p>
              ...
              <form onSubmit={handleSubmit(onSubmitEmail)}>
                ...
                {
                  serveError && (
                    <p className='text-red-600 text-sm mt-2'>{serveError}</p>
                  )}
              </form>
...

          {
            step === "otp" && (
              <>
                ...
                {serveError && (
                  <p className='text-red-500 text-sm mt-2'>{serveError}</p>
                )}
              </>
            )
          }
          ...
          {step === "reset" && (
            ...
            <form onSubmit={handleSubmit(onSubmitPassword)}>
               ...
               <label className='block text-gray-700 mb-1'>Paasword</label>
                 <input type="password"
