...
const Page = () => {
  ...
  const handleSend = async (e: any) => {
    ...
    setChats((prev) => prev.map((c) => c.conversationId ? { ...c, lastMessage: payload.messageBody } : c));
    setMessage("");
    scrollToBottom();
  }

  const getLatestMessage = (chat: any[]) => chat?.lastMessage || "No Messages";
        ...
        <div className='flex h-[80vh] shadow-sm overflow-hidden'>
          <div className='w-[320px] border-r border-r-gray-200 bg-gray-50'>
            ...
            <div className='divide-y divide-gray-200'>
              ...
              ) : chats.length === 0 ? (<div className='p-4 text-sm text-gray-500'>
                No Conversations
              </div>
              ) : (
                chats.map((chat) => {
                  ...
                  return (
                    <button key={chat.conversationId} onClick={() => handleChatSelect(chat)} className={`w - full text - left px - 4 py - 3 transition hover: bg - blue - 500 ${isActive ? "bg-blue-100" : ""}`}>
                      <div className=' flex items-center gap-3'>
                        <Image src={chat.seller?.avatar} alt={chat.seller?.name} width={30} height={30} className="rounded-full border w-[40px] h-[40px] object-cover cursor-pointer" />
                        <div className='flex-1'>
                          <div className='flex items-center justify-between'>
                            <span className='text-sm text-gray-800 font-semibold'>{chat.seller?.name}</span>
                            {chat.seller?.isonline && (<span className='w-2 h-2 rounded-full bg-green-500'></span>)}
                          </div>
                        </div>
                      </div>
                    </button>
            ...
            {selectedChat ? (
              <>
                <div className='p-4 border-b border-b-gray-200 bg-white flex items-center gap-3'>
                  <Image src={selectedChat.seller?.avatar}
                    alt={selectedChat?.seller?.name} height={40} width={40} className='rounded-full border w-[40px] h-[40px] object-cover border-gray-200' />
                  <div>
                    <h2 className='text-gray-800 font-semibold text-base'> {selectedChat.seller?.name}</h2>
                    <p className='text-xs text-gray-500'>{selectedChat.seller?.online ? "online" : "offline"}</p>
                  </div>
                </div>
