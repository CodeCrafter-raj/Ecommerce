...
const CustomSpecifications = ({ control, errors }: any) => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3"> {/* 1. Fixed 'gap-з' to 'gap-3' */}
        {fields?.map((item, index) => (
          <div key={index} className="flex gap-2 items-center">
            <Controller
              ...
              render={({ field }) => ( 
                <Input
                  ...
                  placeholder="e.g.,Batter Life, Weight, Material"
                  {...field}
                />
              )}
            />
            ...
            <button 
              type="button" 
              className='text-red-500 hover:text-red-700' onClick={() => remove(index)} >
              <Trash2 size={20} />
              </button>
          </div>
        ))}
